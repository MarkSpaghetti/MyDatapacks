...
execute as @a[scores={doing=1}] run tellraw @a[scores={revives=1}] {"text": "","extra":[{"selector":"@s","bold": false,"color":"white","clickEvent":{"action":"run_command","value":"/tell @a fuck"},"hoverEvent":{"action":"show_text","value":{"text": "Clicking here will revive the player","color":"#7d3c98"}}}]}
